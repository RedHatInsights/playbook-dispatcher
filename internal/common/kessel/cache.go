...
func NewKesselClientWithCache(client *v1beta2.InventoryClient) *KesselClientWithCache {
	return &KesselClientWithCache{
		...
		permissionCheckFunc: nil, // Will use CheckApplicationPermissions by default
	}
}
...
func (r *rbacClientImpl) GetDefaultWorkspaceIDWithCache(ctx context.Context, orgID string) (string, error) {
	...
	// NOTE: singleflight.Do uses the first caller's context; backend calls are
	// still bounded by the RBAC/Kessel client timeout configured at client construction.
	result, err, shared := r.workspaceFlight.Do(cacheKey, func() (interface{}, error) {
		return r.GetDefaultWorkspaceID(ctx, orgID)
	})
	if err != nil {
		return "", err
	}
...
func (k *KesselClientWithCache) CheckApplicationPermissionsWithCache(ctx context.Context, workspaceID string, serviceFilter string, log *zap.SugaredLogger) ([]string, error) {
	...
	// Validate critical fields required for cache operation
	if reqID == "" {
	...
	result, err, shared := k.applicationFlight.Do(cacheKey, func() (interface{}, error) {
		...
		return checkFunc(ctx, workspaceID, serviceFilter, log)
	})
	if err != nil {
		return nil, err
	}
