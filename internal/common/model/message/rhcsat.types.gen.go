...
func (j *PlaybookSatRunResponseMessageYamlEventsElem) UnmarshalJSON(b []byte) error {
	...
	if _, ok := raw["correlation_id"]; raw != nil && !ok {
	...
	if plain.ConnectionCode != nil && 0 > *plain.ConnectionCode {
		return fmt.Errorf("field %s: must be >= %v", "connection_code", 0)
	}
	if plain.ExecutionCode != nil && 0 > *plain.ExecutionCode {
		return fmt.Errorf("field %s: must be >= %v", "execution_code", 0)
	}
	if plain.SatelliteConnectionCode != nil && 1 < *plain.SatelliteConnectionCode {
		return fmt.Errorf("field %s: must be <= %v", "satellite_connection_code", 1)
	}
	if plain.SatelliteConnectionCode != nil && 0 > *plain.SatelliteConnectionCode {
		return fmt.Errorf("field %s: must be >= %v", "satellite_connection_code", 0)
	}
	if plain.SatelliteInfrastructureCode != nil && 1 < *plain.SatelliteInfrastructureCode {
		return fmt.Errorf("field %s: must be <= %v", "satellite_infrastructure_code", 1)
	}
	if plain.SatelliteInfrastructureCode != nil && 0 > *plain.SatelliteInfrastructureCode {
		return fmt.Errorf("field %s: must be >= %v", "satellite_infrastructure_code", 0)
	}
