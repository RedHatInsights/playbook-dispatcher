	...
	err = this.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if requestType == satMessageHeaderValue {
			...
			if len(existingRunnerEvents) > 0 && len(runnerEvents) > 0 && runnerEvents[0].Event != message.EventExecutorOnStart {
				...
				runnerEvents = mergeRunnerEvents(existingRunnerEvents, *value.RunnerEvents)
...
func mergeRunnerEvents(
	...
	for _, e := range existing {
		// record the "seen" event IDs already in the DB
...
// checkForMissingAndDuplicateEvents takes a slice of events, reads the counters, sorts them,
